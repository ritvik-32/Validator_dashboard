...
fetch_atom_price() {
    local price_data
    price_data=$(curl -s "https://api.coingecko.com/api/v3/simple/price?ids=cosmos&vs_currencies=usd")
    echo "$price_data" | jq -r '.cosmos.usd'
}

# Fetch ATOM token price
...
if [ "$TOKEN_PRICE" == "null" ] || [ -z "$TOKEN_PRICE" ]; then
    ...
    TOKEN_PRICE=$(psql -U "$PGUSER" -d "$PGDATABASE" -h "$PGHOST" -t -c "SELECT price FROM cosmos_data ORDER BY timestamp DESC LIMIT 1;" 2>/dev/null | awk '{print $1}' | head -1)
...
##
echo validator_commision: $VALIDATOR_COMMISSION $AMOUNT_VALUE
